...
const DeckService = {
    async createDeck(deckId, playerId, name, cards) {
    ...
    `);
        ...
        const transaction = db.transaction(() => {
            ...
            for (const card of cards) {
                if (card.quantity <= 0)
                    continue; // Skip if quantity is zero or less
    ...
    async fetchDeck(deckId) {
        ...
        try {
            ...
            const deckResult = deckQuery.get(deckId);
            if (!deckResult) {
            ...
            const cardResults = cardsQuery.all(deckId);
            ...
            if (error instanceof DeckNotFoundError)
                throw error;
            throw new Error(`An unexpected error occurred while fetching deck ${deckId}: ${error.message}`);
        }
    },
    async getPlayerDeckCards(deckId, cardDatabase) {
     ...
     */
    validateDeck(deckList, cardDatabase) {
