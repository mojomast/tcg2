...
export class ResourceManager {
     ...
     */
    addMana(playerId, color, amount = 1) {
    ...
    clearManaPool(playerId) {
        ...
        Object.keys(pool).forEach(color => {
    ...
    canPayCost(playerId, cost) {
        ...
        const availableManaForGeneric = { ...pool };
        ...
        for (const color of requiredColors) {
            ...
            if (requiredAmount && requiredAmount > 0) {
                const availableAmount = availableManaForGeneric[color] || 0;
        ...
        for (const color in availableManaForGeneric) {
            totalAvailableForGeneric += availableManaForGeneric[color] || 0;
        }
    ...
    canAffordCost(playerId, cost) {
        ...
        for (const color in cost) {
            ...
            const availableAmount = pool[color] ?? 0;
            if (availableAmount < requiredAmount) {
     ...
     */
    spendMana(playerId, color, amount) {
        // Find the correct player state (assuming players is an array)
        const playerState = this.gameState.players.find(p => p.playerId === playerId);
        if (!playerState) {
     ...
     */
    tapForMana(playerId, permanentId) {
    ...
    untapPermanents(playerId) {
        ...
        Object.keys(playerState.battlefield).forEach(zoneType => {
     ...
     */
    playResource(playerId, cardObjectId) {
        const playerState = this.dependencies.getPlayerStateFn(playerId);
        if (!playerState) {
        ...
        const cost = card.cost || { C: 0 }; // Assume 0 if no cost defined
        ...
        if (gameObject) {
            gameObject.tapped = true; // Assert using BattlefieldCard
    ...
    payCost(playerId, cost) {
        ...
        let remainingGenericCost = cost.C ?? 0;
        const manaToSpendForGeneric = { ...pool }; // Work on a copy
        ...
        for (const color of requiredColors) {
            ...
            if (requiredAmount && requiredAmount > 0) {
                ...
                if (manaToSpendForGeneric[color]) {
                    manaToSpendForGeneric[color] = manaToSpendForGeneric[color] - requiredAmount;
                }
            }
        }
        if (!paidSuccessfully)
            return false; // Exit if colored mana payment failed
            ...
            if (paidSuccessfully && remainingGenericCost > 0) {
                for (const color of requiredColors) { // Iterate through WUBRG
                    ...
                    if (availableAmount > 0) {
                        ...
                        if (remainingGenericCost <= 0)
                            break; // Stop if generic cost is fully paid
                    }
                }
            }
