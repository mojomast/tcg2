...
export class StateManager {
    constructor(engine, gameState, getCardFn) {
    ...
    checkStateBasedActions() {
        ...
        while (continueChecking) {
            ...
            if (continueChecking)
                continue; // Restart checks if an action occurred
            // 2. Creature health <= 0 or marked lethal damage check
            this.gameState.players.forEach(player => {
                const creaturesToRemove = [];
                const objectsToMove = []; // Prepare for moving
     ...
     */
    moveCardToZone(playerId, cardObjectId, fromZone, toZone) {
        ...
        if (fromZone === 'stack') {
            ...
            const battlefieldZones = ['creatures', 'resources', 'enchantments', 'artifacts', 'planeswalkers', 'other']; // Add other relevant zones
            ...
            const sourceZoneArray = playerState[fromZone];
        ...
        if (toZone === 'stack') {
            ...
            let targetBattlefieldZone = null;
            switch (baseCard.type) {
                case 'Creature':
                    targetBattlefieldZone = 'creatures';
                    break;
                case 'Artifact':
                    targetBattlefieldZone = 'artifacts';
                    break;
                case 'Enchantment':
                    targetBattlefieldZone = 'enchantments';
                    break;
                case 'Planeswalker':
                    targetBattlefieldZone = 'planeswalkers';
                    break;
                case 'Land':
                    targetBattlefieldZone = 'resources';
                    break; // Assuming Lands go to 'resources'
            ...
            const targetZoneArray = playerState[toZone];
