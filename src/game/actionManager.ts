import { GameState, PlayerId, GameObjectId, StackItem, ManaColor, EventType } from '../interfaces/gameState.js';
import { Card, Ability } from '../interfaces/card.js';
import { GameEngine } from './gameEngine.js'; // May be needed for accessing engine methods or state
import { TurnManager } from './turnManager.js'; // Import TurnManager
...
export class ActionManager {
    ...
    private engine: GameEngine; // Reference to the main engine if needed
    private turnManager: TurnManager; // Reference to TurnManager

    constructor(gameState: GameState, engine: GameEngine, turnManager: TurnManager) {
     ...
     */
    playCard(playerId: PlayerId, cardObjectId: GameObjectId, targets?: GameObjectId[]): void {
     ...
     */
    activateAbility(playerId: PlayerId, sourceInstanceId: GameObjectId, abilityId: string, targets?: GameObjectId[]): boolean {
     ...
     */
    public resolveTopStackItem(): void {
        if (this.gameState.stack.length === 0) {
        ...
        if (topItem.type === 'Spell') {
            ...
            if (['Creature', 'Artifact', 'Enchantment', 'Planeswalker'].includes(baseCard.type)) {
                ...
                const success = this.engine.stateManager.moveCardToZone(
                    topItem.controllerId,
                    topItem.sourceInstanceId, // The ID of the specific card instance played from hand
    ...
    */
     ...
     */
    public discardCard(playerId: PlayerId, cardObjectId: GameObjectId): boolean {
