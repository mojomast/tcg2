...
export class TurnManager {
    ...
    private engine: GameEngine; // Keep a reference to the engine for shared functionalities
    private resourceManager: ResourceManager;
    private combatManager: CombatManager | null = null; // Can be null if not set immediately
    private stateManager: StateManager;

    // Standard MTG turn structure
    public static readonly phaseOrder: GamePhase[] = ['BEGIN', 'MAIN', 'COMBAT', 'MAIN', 'END'];
    public static readonly stepOrder: { [key in GamePhase]?: GameStep[] } = {
    ...
    constructor(gameState: GameState, engine: GameEngine, resourceManager: ResourceManager, stateManager: StateManager) {
    ...
    public getCurrentPhaseSteps(): GameStep[] {
    ...
    private getCurrentPhaseStepsForPhase(phase: GamePhase): GameStep[] {
        if (phase === 'MAIN') return TurnManager.stepOrder['MAIN']!;
    ...
    private untapStepAction(playerId: PlayerId): void {
    ...
    private drawStepAction(playerId: PlayerId): void {
    ...
    private cleanupStepAction(activePlayerId: PlayerId): void {
        ...
        this.gameState.players.forEach(player => {
            Object.values(player.battlefield).flat().forEach((card: BattlefieldCard) => {
