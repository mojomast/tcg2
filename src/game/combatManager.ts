import { GameState, PlayerId, BattlefieldCard, GameObjectId, GameStep, GamePhase, Keyword } from '../interfaces/gameState';
import { TurnManager } from './turnManager'; // Import TurnManager type
import { StateManager } from './stateManager'; // Need for checking SBAs
import { Card } from '../interfaces/card'; // Need for Card type

type ValidateActionFn = (playerId: PlayerId, requiredStep?: GameStep | null, requiredPhase?: GamePhase | null) => boolean;
type GetCardFn = (instanceId: GameObjectId) => Card | undefined;
type FindBattlefieldCardFn = (instanceId: GameObjectId) => BattlefieldCard | undefined;
type HasKeywordFn = (instanceId: GameObjectId, keyword: Keyword) => boolean;

export class CombatManager {
    private gameState: GameState;
    private turnManager: TurnManager | null; // Allow null initially
    private stateManager: StateManager;
    private validatePlayerActionFn: ValidateActionFn;
    private getCardFromInstanceIdFn: GetCardFn;
    private findBattlefieldCardFn: FindBattlefieldCardFn;
    private hasKeywordFn: HasKeywordFn;

    constructor(
        gameState: GameState, 
        turnManager: TurnManager | null, // Accept null
        stateManager: StateManager,
        validateFn: ValidateActionFn,
        getCardFn: GetCardFn,
        findBattlefieldFn: FindBattlefieldCardFn,
        hasKeywordFn: HasKeywordFn
    ) {
    ...
    public declareAttackers(playerId: PlayerId, attackers: { [attackerId: string]: PlayerId }): void {
    ...
    public declareBlockers(playerId: PlayerId, blockers: { [blockerId: string]: string }): void {
    ...
    public assignCombatDamage(isFirstStrikeStep: boolean): void {
        ...
        Object.keys(attackers).forEach(attackerId => {
            ...
            if (!attackerBattlefieldCard || !attackerCard || attackerCard.type !== 'Creature') return;
            ...
            Object.keys(blockers).forEach(blockerId => {
                if (blockers[blockerId] === attackerId) {
                    ...
                    if (!blockerBattlefieldCard || !blockerCard || blockerCard.type !== 'Creature') return;
    ...
    public checkKeywords(...keywords: string[]): boolean {
    ...
    // --- Helper methods (potentially moved from GameEngine2) ---
    // Example:
    // private getCombatCreatures(): BattlefieldCard[] { ... }
}
